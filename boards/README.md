...
| **B** | `Board_B_Revenue_Automation_Engine_v6_19.mmd` | `flowchart LR` | Builder | GHL implementation: workflow names, step counts, triggers, tags, calendar IDs |
...
- **v6.19 (2026-09-09):** Board B only — O.0 Objection Handler corrected from `DRAFT | 142 steps | RECONNECT TRIGGERS` to **`PUBLISHED v157 | 588 step records | TRIGGER ACTIVE`** (ID `fdf4ad82-33ab-4e73-b581-18d21d51ac42`), verified against the live GHL API at 22:30Z. The stale claim came from the pre-rename S4.10 record. Added: the objection tag family is the **hyphen** form (`objection-confirmed-competitor`) — a colon-form tag matches no workflow; and **O.RR was never built** (no such workflow exists in the location) and is superseded by the O.0 competitor branch, which is the rescission rescue arc. Boards A and C reference O.0 only as a routing destination and carry no status claim, so neither changed.
